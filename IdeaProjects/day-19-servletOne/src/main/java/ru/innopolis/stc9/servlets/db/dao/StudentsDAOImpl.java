...
public class StudentsDAOImpl implements StudentsDAO {
    ...
    @Override
    public boolean checkForduplicate(Students student) {
        Connection connection = connectionManager.getConnection();
        try {

            try (PreparedStatement statement = connection.prepareStatement("SELECT * " +
                    "FROM students WHERE name = ? AND email = ? INNER JOIN users ON students.id = users.id" +
                    " WHERE users.id = ? AND login = ?")) {
                statement.setString(1, student.getName());
                statement.setString(2, student.getEmail());
                statement.setInt(3, student.getUsers_id());
                try (ResultSet resultSet = statement.executeQuery()) {


            if (resultSet.next()) {

                loggerFileInf.info("the same Student was found. id:" + resultSet.getString("id"));
                return true;
            }
                }
            }
